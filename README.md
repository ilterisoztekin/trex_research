...
Bu çalışmada amaç; verilen bir haber metninin veya başlığının ait olduğu kategoriyi (Dünya, Spor, Ticaret, Bilim/Teknoloji) otomatik olarak tahmin eden bir metin sınıflandırma sistemi kurmaktır. Proje kapsamında **AG News Classification Dataset** kullanılmış, dengeli alt örnekleme (balanced downsampling) yapılarak 3 farklı model eğitilmiş ve performansları karşılaştırılmıştır.

### Kullanılan Araçlar ve Yöntemler (NLP Pipeline)

1. **Metin Temizleme (Text Preprocessing):** Harfler küçültülmüş (lowercase) ve veri setindeki noktalama işaretleri temizlenerek öznitelik kirliliği önlenmiştir.

2. **TF-IDF Vektörleştirme:** Kelimelerin ayırt edici güçlerine göre ağırlık hesaplayan istatistiksel Bag-of-Words yöntemiyle metinler sayısal matrislere dönüştürülmüştür.

3. **Tokenization:** DistilBERT modeli için metinler alt kelime (sub-word) birimlerine ayrılmıştır.

---

### Model Çıktılarının Karşılaştırılması ve Metrikler

Google Colab üzerinde dengeli alt küme (Eğitim: 4000, Test: 1000 satır) ile yapılan deneyler sonucunda elde edilen **gerçek** Precision, Recall ve F1-Score değerleri aşağıdaki tabloda belgelenmiştir:

| Model Mimarisi | Precision (Keskinlik) | Recall (Duyarlılık) | F1-Score (Harmonik Ort.) | Accuracy (Genel Doğruluk) |
| :--- | :---: | :---: | :---: | :---: |
| **Model 1: Naive Bayes** | %0.84 | %0.83 | %0.83 | %0.83 |
| **Model 2: Logistic Regression** | %0.88 | %0.88 | %0.88 | %0.88 |
| **Model 3: DistilBERT (Fine-Tuned)** | **%0.91** | **%0.91** | **%0.91** | **%0.91** |

---

### Değerlendirme ve Teknik Yorumlar


#### 1. En İyi Sonucu Veren Model: DistilBERT (%91 Başarı)

* **Neden En İyisi?**
 
   Geleneksel modeller (Naive Bayes ve Logistic Regression), metinleri "Bag-of-Words" (Kelimeler Torbası) mantığıyla işler. Yani kelimelerin cümle içindeki sırasını, eklerini ve en önemlisi **bağlamını (context)** tamamen göz ardı eder. 
* DistilBERT ise bir Transformer mimarisidir. Sahip olduğu **Self-Attention (Öz-Dikkat)** mekanizması sayesinde kelimelerin birbirleriyle olan anlamsal ilişkilerini ve cümlenin yapısını bütünsel olarak kavrar. Ayrıca daha önce çok büyük veri kümelerinde ön eğitimden geçtiği için (Pre-trained), dildeki karmaşık kalıpları çok daha iyi ayırt ederek %91'lik en yüksek başarıya ulaşmıştır.

#### 2. En Düşük Sonucu Veren Model: Naive Bayes (%83 Başarı)

* **Neden En Düşük?** 

   Naive Bayes algoritması, metindeki tüm kelimelerin birbirinden **tamamen bağımsız** olduğunu varsayar (Sınıf koşullu bağımsızlık kabulü). Ancak gerçek dilde kelimeler bağımsız değildir, ardışık dizilimleri anlamı doğrudan değiştirir. Algoritmanın bu "saf/sade" varsayımı bağlam kaçırmasına neden olmuş ve %83 ile diğer modellerin gerisinde kalmasına yol açmıştır. Buna rağmen CPU üzerinde saniyeler içinde bu başarıyı yakalaması, onu harika bir baseline (referans) model yapmaktadır.

#### 3. Ara Model: Logistic Regression (%88 Başarı)

* Logistic Regression, doğrusal bir sınıflandırıcı olarak TF-IDF matrisindeki yüksek boyutlu ve seyrek (sparse) verileri çok iyi optimize etmiştir. DistilBERT gibi devasa bir GPU gücüne veya uzun eğitim sürelerine ihtiyaç duymadan, CPU üzerinde anlık bir şekilde %88 gibi oldukça güçlü ve endüstri standardında kabul edilebilir bir performans sergilemiştir.
